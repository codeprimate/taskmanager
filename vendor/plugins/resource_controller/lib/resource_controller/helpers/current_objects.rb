module ResourceController::Helpers::CurrentObjects
  protected
    ...
    # helper for form fileds for polymorphic views
    #
    def object_name
      @object.type.to_s.tableize.singularize
    end
    
    # a helper method to return to the view a humanized form of the model name
    #
    def object_human_name
      model.to_s.tableize.humanize
    end
    
    # helper to find parent object using generic syntax
    # when nested will return the direct parent
    def parent_object
      parent? ? parent_objects.last.last : nil
    end
