    ...
    module Base
      ...
      protected
        ...
        def options_from_pagination_state(pagination_state)
          find_options = { :offset => pagination_state[:offset], :limit  => pagination_state[:limit] }
          find_options.merge!(:order => "#{pagination_state[:sort_field]} #{pagination_state[:sort_direction]}") unless pagination_state[:sort_field].blank?    
          find_options
        end
