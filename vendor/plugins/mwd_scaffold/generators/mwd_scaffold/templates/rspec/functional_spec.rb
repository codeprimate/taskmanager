...
describe <%= controller_class_name %>Controller do
  ...
  describe "handling GET /<%= table_name %>" do
    ...
    it "should find all <%= table_name %>" do
      <%= controller_class_name.singularize %>.should_receive(:find).with(:all, {:limit=>25}).and_return([@<%= file_name %>])
      do_get
    end
  ...
  describe "handling DELETE /<%= table_name %>/1" do
    ...
    before(:each) do
      @<%= file_name %> = mock_model(<%= controller_class_name.singularize %>, :delete! => true)
      <%= controller_class_name.singularize %>.stub!(:find).and_return(@<%= file_name %>)
    end
    ...
    it "should call destroy on the found <%= file_name %>" do
      @<%= file_name %>.should_receive(:delete!).and_return(true)
      do_delete
    end
